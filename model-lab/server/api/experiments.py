...
def _load_experiment(experiment_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if stored_prov and stored_prov.get("algorithm") == "sha256":
            req_canonical = json.dumps(request, sort_keys=True, separators=(",", ":")).encode(
                "utf-8"
            )
...
async def create_experiment(
    ...
    candidate_ids: str | None = Form(None),  # comma-separated, optional
    config: str | None = Form(None),  # JSON config overrides
        ...
        for cid in parsed_candidate_ids:
            ...
            if not c:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "error_message": f"Candidate not found: {cid}",
                    },
                )
            if c.use_case_id != use_case_id:
                return JSONResponse(
                    status_code=400,
                    content={
                        "error_code": "CANDIDATE_USE_CASE_MISMATCH",
                        "error_message": f"Candidate {cid} is for {c.use_case_id}, not {use_case_id}",
                    },
                )
    ...
    if custom_steps or pipeline_template_name or preprocessing_ops:
        try:
            ...
            return JSONResponse(
                status_code=400, content={"error_code": "INVALID_PIPELINE", "error_message": str(e)}
            )
...
def start_next_run(experiment_id: str) -> JSONResponse:
    ...
    if not queued_slot:
        return JSONResponse(
            status_code=200, content={"started": False, "message": "No QUEUED candidates"}
        )
    ...
    candidate_config = next(
        (c for c in request["candidates"] if c["candidate_id"] == queued_slot["candidate_id"]), None
    )
...
def compare_experiment_runs(
    ...
    if artifact not in allowed_artifacts:
        raise HTTPException(
            status_code=400, detail=f"Invalid artifact. Allowed: {allowed_artifacts}"
        )
    ...
    if (
        left_data.get("error") == "PREVIEW_TOO_LARGE"
        or right_data.get("error") == "PREVIEW_TOO_LARGE"
    ):
        return JSONResponse(
            status_code=413,
            content={"error_code": "PREVIEW_TOO_LARGE", "left": left_data, "right": right_data},
        )
