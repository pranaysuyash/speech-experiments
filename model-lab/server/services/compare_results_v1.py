...
def compute_comparison_v1(
    experiment_id: str, res_a: dict | None, res_b: dict | None
) -> dict[str, Any]:
    """
    ...
    summary: dict[str, Any] = {
        ...
        "readiness": {"comparable": False, "reason": "MISSING_RESULTS"},
        "metrics": {},
        "verdicts": {"overall": "PENDING", "reasons": []},
        "provenance": {"computed_at": datetime.now().isoformat(), "results_schema": "v1"},
    }
    ...
    for key in ["word_count", "duration_s", "confidence_avg"]:
        ...
        if val_a is not None and val_b is not None:
            ...
            metrics[key] = {"A": val_a, "B": val_b, "delta": delta, "pct_change": pct}
    ...
    if status_a == "COMPLETED" and status_b == "FAILED":
        ...
        if conf:
            ...
            if delta_conf > 0.05:
                verdict = "A_BETTER"
                reasons.append(
                    f"A has significantly higher confidence (+{conf['pct_change']:.1f}%)."
                )
                winner_declared = True
            elif delta_conf < -0.05:
                verdict = "B_BETTER"
                reasons.append(
                    f"B has significantly higher confidence (A is {conf['pct_change']:.1f}%)."
                )
                winner_declared = True
        ...
        if not winner_declared:
            ...
            if wc:
                ...
                if delta_wc_pct > 10:
                    ...
                    verdict = "B_BETTER"
                    reasons.append(
                        f"B produced significantly more content (A is {delta_wc_pct:.1f}%)."
                    )
                    winner_declared = True
