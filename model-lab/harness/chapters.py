...
class SemanticSegmenter:
    ...
    def segment(
        self, segments: list[AlignedSegment], duration: float
        ...
        if duration < 120.0:
            logger.warning(
                f"Audio too short for segmentation ({duration:.1f}s < 120s). Returning single chapter."
            )
            return [self._create_chapter(0, 0.0, duration, segments)], {
                **config,
                "gating": "short_duration",
            }
        ...
        for i in range(len(embeddings) - 1):
            sim = cosine_similarity(embeddings[i].reshape(1, -1), embeddings[i + 1].reshape(1, -1))[
                0
            ][0]
            sims.append(sim)
    ...
    def _create_windows(self, segments, duration) -> list[dict]:
        ...
        while curr_time + self.WINDOW_SIZE_SEC <= duration + self.STRIDE_SEC:  # inclusive-ish
            ...
            if combined_text:
                windows.append({"start": curr_time, "end": end_time, "text": combined_text})
    ...
    if result.success:
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Failed to parse LLM response for chapter {chapter.index}"
            )
    else:
        logging.getLogger(__name__).warning(
            f"LLM failure for chapter {chapter.index}: {result.error_message}"
        )


def run_chapters(
    alignment_path: Path, output_dir: Path, cache_dir: Path | None = None, force: bool = False
) -> Path:
    """
