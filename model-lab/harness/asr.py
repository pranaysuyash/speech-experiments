...
def resolve_asr_config(user_config: dict[str, Any] | None = None) -> ResolvedASRConfig:
    """
    ...
    return ResolvedASRConfig(
        model_id=model_id, source=source, device=actual_device, reason=reason, language=language
    )
...
def run_asr(
    ...
    )  # Still need type for loader? resolve_asr_config output has model_id "type:name"
    ...
    ingest = ingest_media(
        input_path, output_dir, IngestConfig()
    )  # Use default config for re-ingest?
    ...
    run_ctx = create_run_context(
        task="asr", model_id=f"{model_type}:{model_name}", grade="adhoc", device=device
    )
    ...
    artifact = RunnerArtifact(
        ...
        metrics_structural={"duration_ms": duration_ms, "rtf": rtf, "latency_ms": duration_ms},
        gates={},
    )
