...
from harness.transcript_view import TranscriptView, Segment
...
def chunk_by_time(view: TranscriptView, policy: ChunkingPolicy) -> List[Chunk]:
    ...
    for seg in view.segments:
        ...
        should_split = (
            current_segments and (
                current_duration + seg.duration_s > policy.max_chunk_seconds or
                len(new_text) > policy.max_chunk_chars
            )
        )
    ...
    for sent in sentences:
        ...
        if len(new_text) > policy.max_chunk_chars and current_sentences:
            ...
            if len(chunk_text) >= policy.min_chunk_chars:
                ...
                start_ratio = sum(len(' '.join(current_sentences[:i])) for i in range(len(chunks))) / len(text) if text else 0
...
def chunk_transcript(view: TranscriptView, policy: Optional[ChunkingPolicy] = None) -> ChunkingResult:
    ...
    if len(view.full_text) <= policy.max_chunk_chars and view.duration_s <= policy.max_chunk_seconds:
