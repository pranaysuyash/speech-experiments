...
class VADMetrics:
    @staticmethod
    def calculate(
        segments: list[dict[str, int]], total_duration_samples: int, sr: int = 16000
    ) -> VADMetricsResult:
        """
        ...
        return VADMetricsResult(
            speech_ratio=speech_ratio, num_segments=num_segments, avg_segment_duration=avg_dur
        )
    ...
    def check_gates(metrics: VADMetricsResult, expectations: dict[str, Any]) -> dict[str, Any]:
        ...
        if "expected_speech_ratio_min" in expectations:
            gates["speech_ratio_min"] = (
                metrics.speech_ratio >= expectations["expected_speech_ratio_min"]
            )

        # Check max speech ratio
        if "expected_speech_ratio_max" in expectations:
            gates["speech_ratio_max"] = (
                metrics.speech_ratio <= expectations["expected_speech_ratio_max"]
            )
