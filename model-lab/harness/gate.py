...
from .results import BatchResult, ResultsManager
...
class PromotionCriteria:
    ...
    baseline_model_id: Optional[str] = None  # Model to compare against
...
class ProductionGate:
    ...
    def _check_min_tests(self, summary: Dict[str, Any]) -> GateCheck:
        ...
        return GateCheck(
            ...
            if not passed else f"✓ {total} test cases"
        )
    ...
    def _check_success_rate(self, summary: Dict[str, Any]) -> GateCheck:
        ...
        return GateCheck(
            ...
            if not passed else f"✓ {rate:.1%} success rate"
        )
    ...
    def _check_wer(self, summary: Dict[str, Any]) -> GateCheck:
        ...
        return GateCheck(
            ...
            if not passed else f"✓ WER {wer:.3f}"
        )
    ...
    def _check_latency(self, summary: Dict[str, Any]) -> GateCheck:
        ...
        return GateCheck(
            ...
            if not passed else f"✓ Latency {latency:.0f}ms"
        )
    ...
    def _check_baseline(self, model_id: str, task_type: str) -> GateCheck:
        ...
        return GateCheck(
            ...
            if not passed else f"✓ Beats baseline ({current_wer:.3f} <= {baseline_wer:.3f})"
        )
    
    def _get_model_status(self, model_id: str) -> str:
        """Get current model status from registry."""
        return self.registry.get('models', {}).get(model_id, {}).get(
