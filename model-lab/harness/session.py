...
from harness.errors import classify_error, format_traceback
...
class SessionContext:
    ...
    update_progress: Callable[[int, str | None, int | None], None] | None = None
    ...
    @property
    def audio_fingerprint(self) -> str | None:
        if not self.ingest:
            return None
...
class SessionRunner:
    ...
    def get_artifact(self, type_name: str, step_hint: str | None = None) -> Path | None:
        """
    ...
    def _get_step_artifact(self, step_name: str, extension: str = ".json") -> Path | None:
    ...
    def _write_eval_json(self, manifest: dict[str, Any]) -> None:
        ...
        if (
            eval_mode == "enriched_v0"
        ):  # Force new mode if strictly needed, but let's stick to "enriched" or just update default.
        ...
        checks.append(
            {
                ...
                "severity": "info",
                "message": "Bundle manifest present"
                if manifest_exists
                else "Bundle manifest not found",
        ...
        checks.append(
            {
                ...
                "message": "Bundle manifest valid JSON"
                if manifest_parseable
                else ("Bundle manifest invalid JSON" if manifest_exists else "N/A - no manifest"),
                "evidence_paths": ["bundle/bundle_manifest.json"] if manifest_exists else [],
            }
        )
        ...
        checks.append(
            {
                ...
                "message": "Run completed successfully" if status_ok else f"Run status: {status}",
            ...
            try:
                if t_path.suffix == ".json":
                    ...
                    if "segments" in data and isinstance(data["segments"], list):
                        t_text = " ".join(
                            seg.get("text", "") for seg in data["segments"] if isinstance(seg, dict)
                        )
                        t_len = len(t_text)
                    elif "text" in data:
                        t_text = data["text"]
        ...
        try:
            if self.manifest_path.exists():
                ...
                if disk_status in terminal_states:
                    ...
                    logger.warning(
                        f"Merge-Safe: Adopted terminal status {disk_status} from disk (Preserved metadata)"
                    )
    ...
    def _register_steps(self) -> None:
        ...
        self.steps["asr"] = StepDef(
            name="asr", deps=["ingest"], func=asr_func, artifact_paths=asr_artifacts
        )
    ...
    def _artifact_exists_and_hash_matches(
        self, artifact_path: Path, expected_hash: str | None
    ) -> bool:
    ...
    def _is_step_valid_for_resume(
        self, m: dict[str, Any], step_name: str, step_def: StepDef
    ) -> bool:
    ...
    def _execute_step(self, m: dict[str, Any], step_def: StepDef) -> None:
            ...
            while not heartbeat_stop.is_set():
                ...
                if not heartbeat_stop.is_set():
                    ...
                    if progress_state["last_semantic_progress_at"]:
                        ...
                        if step_entry:
                            step_entry["last_semantic_progress_at"] = progress_state[
        ...
        try:
            ...
            fail_target = os.environ.get("MODEL_LAB_FAIL_STEP") or os.environ.get(
                "SESSION_FAIL_STEP"
            )
            ...
            if fail_target == step_def.name:
                if is_testing:
                    logger.warning(
                        f"⚠️ INJECTING FAILURE at step '{step_def.name}' (MODEL_LAB_TESTING=1)"
                    )
                    time.sleep(1)  # Ensure started_at is distinct
                    raise RuntimeError(
                        f"Simulated failure for step: {step_def.name} (Triggered by Env Var)"
                    )
                else:
                    logger.info(
                        f"Ignored failure injection request for '{step_def.name}' (MODEL_LAB_TESTING not set)"
                    )
            ...
            if isinstance(raw_result, dict):
                ...
                for item in raw_result:
                    if isinstance(item, (str, Path)):
                        ...
                        raise TypeError(
                            f"Step '{name}' returned list containing invalid type: {type(item)}. Expected str, Path, or dict."
                        )
                ...
                raise TypeError(
                    f"Step '{name}' returned None. Steps must return a dict or artifact path(s)."
                )
            else:
                raise TypeError(
                    f"Step '{name}' returned invalid type: {type(raw_result)}. Expected dict, str, Path, or list."
                )
        ...
        try:
            ...
            for s_name, s_rec in m.get("steps", {}).items():
                if s_rec.get("status") == "RUNNING":
                    logger.warning(
                        f"Found stale RUNNING step '{s_name}'. Marking FAILED (StaleRun)."
                    )
                    s_rec["status"] = "FAILED"
                    s_rec["error"] = {
                        "type": "StaleRun",
                        "message": "Previous run crashed or was interrupted.",
                    }
                    stale_detected = True
            ...
            if m.get("status") != "FAILED":
                ...
                try:
                    ...
                    if disk_m.get("status") in ("CANCELLED", "FAILED", "STALE"):
                        ...
                        logger.warning(
                            f"Run completion preempted by external status: {m['status']}"
                        )
                    else:
                        m["status"] = "COMPLETED"
                except Exception:
                ...
                for step_name, step_rec in m.get("steps", {}).items():
                    ...
                    if step_rec.get("status") == "COMPLETED":
                        for art in step_rec.get("artifacts", []):
                            ...
                            if p.exists():
                                rel_path = (
                                    p.relative_to(self.session_dir)
                                    if p.is_relative_to(self.session_dir)
                                    else f"artifacts/{p.name}"
                                )
                                zf.write(p, rel_path)
