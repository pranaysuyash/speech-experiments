...
class SessionRunner:
    ...
    def _register_steps(self) -> None:
        ...
        def alignment_func(ctx: SessionContext) -> Dict[str, Any]:
            ...
            if not asr_path:
                 # Legacy fallback: find any asr_*.json in artifacts dir
                 asr_candidates = sorted(ctx.artifacts_dir.glob("asr_*.json"))
                 if asr_candidates:
                     asr_path = asr_candidates[0]
                 else:
                     asr_path = ctx.artifacts_dir / "asr.json"
            
            diar_path = self.get_artifact("diarization")
            if not diar_path:
                 # Legacy fallback: check diarization subdirectory too
                 diar_sub = ctx.artifacts_dir / "diarization"
                 diar_candidates = sorted(diar_sub.glob("diarization_*.json")) if diar_sub.is_dir() else []
                 if diar_candidates:
                     diar_path = diar_candidates[0]
                 else:
