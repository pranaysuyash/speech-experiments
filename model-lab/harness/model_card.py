...
from .taxonomy import EvidenceGrade, TaskRole, TaskType
...
class HardwareInfo:
    ...
    ram_gb_recommended: float | None = None
    disk_gb_model: float | None = None
...
class EvidenceEntry:
    ...
    device: str | None = None  # e.g. "mps"
    run_date: str | None = None
    verified_at: str | None = None  # ISO date
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        from enum import (
            Enum,
        )  # Import locally to avoid circular dependency if Taxonomy imports ModelCard

        return {
            "task": self.task.value if isinstance(self.task, Enum) else self.task,
            "dataset_id": self.dataset_id,
            "evidence_grade": self.evidence_grade.value
            if isinstance(self.evidence_grade, Enum)
            else self.evidence_grade,
...
class ModelCard:
    ...
    def from_sources(
        ...
        config: dict[str, Any],
        runs_summary: dict[str, Any] | None = None,
        ...
        hardware = HardwareInfo(
            accelerators_supported=registry_meta.get(
                "hardware", config_hw.get("accelerators_supported", ["cpu"])
            ),
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "artifacts": [
                {"type": a.type, "precision": a.precision, "notes": a.notes} for a in self.artifacts
            ],
            "install": {
            ...
            "links": {"paper_url": self.paper_url, "repo_url": self.repo_url},
        }
