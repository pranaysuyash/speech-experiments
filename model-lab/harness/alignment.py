...
from harness.transcript_view import Segment, from_asr_artifact
...
class AlignmentMetrics:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "speaker_distribution": {k: round(v, 2) for k, v in self.speaker_distribution.items()},
        }
...
def align_segments(
    asr_segments: list[Segment], diarization_turns: list[dict]
) -> list[AlignedSegment]:
    """
    ...
    for seg in asr_segments:
        ...
        for turn in candidates:
            ...
            overlap = calculate_overlap(
                seg.start_s, seg.end_s, turn.get("start", 0), turn.get("end", 0)
            )
...
def align_artifacts(asr_path: str, diarization_path: str | None) -> AlignedTranscript:
    """
...
def run_alignment(
    asr_path: Path, diarization_path: Path, output_dir: Path, force: bool = False
) -> Path:
    """
    Run alignment task.
    ...
    if not diar_path_str:
        logger.warning(
            f"Diarization not found: {diarization_path}. Proceeding without speaker attribution."
        )
