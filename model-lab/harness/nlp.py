...
def run_summarize_by_speaker(
    alignment_path: Path, output_dir: Path, nlp_model: str = DEFAULT_NLP_MODEL, force: bool = False
) -> Path:
    ...
    artifact_path = (
        output_dir / f"summary_by_speaker_{alignment_path.stem.replace('alignment_', '')}.json"
    )
...
def extract_pass_b(
    candidates: list[dict], context_text: str, allowed_speakers: list[str], nlp_model: str
) -> list[dict]:
    if not candidates:
        return []
...
def run_action_items(
    ...
    artifact_path = (
        output_dir / f"action_items_{alignment_path.stem.replace('alignment_', '')}.json"
    )
