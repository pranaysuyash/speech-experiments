...
class TestMandatoryProvenance:
    ...
    def test_all_runs_have_provenance(self):
        ...
        for run_file in runs_dir.glob("**/*.json"):
            ...
            if "runs/experiments/" in str(run_file).replace("\\", "/") and run_file.name in (
        ...
        if violations:
            ...
            pytest.fail(
                f"Runs without provenance ({len(violations)}):\n" + "\n".join(violations[:10])
            )
    ...
    def test_no_wer_without_ground_truth(self):
        ...
        if violations:
            pytest.fail("WER without ground truth:\n" + "\n".join(violations))
        ...
        for run_file in runs_dir.glob("**/v2v/*.json"):
            ...
            try:
                ...
                if prov.get("has_ground_truth") is False:
                    ...
                    if not has_finite:
                        violations.append(
                            f"{run_file}: V2V smoke missing structural metrics (latency/rtf)"
                        )
            except (json.JSONDecodeError, KeyError):
                pass

        if violations:
            pytest.fail("V2V smoke without structural metrics:\n" + "\n".join(violations))
                ...
                if prov.get("has_ground_truth") is False:
                    for field in quality_fields:
                        if metrics.get(field) is not None:
                            violations.append(
                                f"{run_file}: has_ground_truth=False but {field}={metrics[field]}"
                            )
            except (json.JSONDecodeError, KeyError):
                pass

        if violations:
            pytest.fail("V2V quality metrics without ground truth:\n" + "\n".join(violations))
