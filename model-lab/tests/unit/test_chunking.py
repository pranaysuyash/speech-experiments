...
from harness.chunking import (
    ChunkingPolicy, Chunk, ChunkingResult,
    chunk_transcript, chunk_by_time, chunk_by_text,
    dedupe_items,
)
...
class TestChunking:
    ...
    def test_long_text_chunked(self):
        ...
        for chunk in result.chunks:
            assert len(chunk.text) <= policy.max_chunk_chars + 100  # Some tolerance for sentence boundaries
    ...
    def test_chunk_respects_max_seconds_with_segments(self):
        ...
        segments = [
            Segment(start_s=i * 10, end_s=(i + 1) * 10, text=f"This is segment number {i} with some extra words to make it longer.")
            for i in range(12)
        ]
