...
class TestFailureStepCapture(unittest.TestCase):
    ...
    def test_pipeline_step_failure_captures_authoritative_failure_step(self):
        ...
        with patch.object(runner, "_execute_step", side_effect=mock_execute_step), \
        ...
        completed_steps = [name for name, data in manifest["steps"].items() 
    ...
    def test_early_failure_sets_failure_step_to_null(self):
        ...
        with patch.object(runner, "_topo_order", side_effect=mock_topo_order), \
             patch.object(runner, "_export_partial_bundle"), \
             patch("harness.meeting_pack.build_meeting_pack"):
        ...
        self.assertEqual(manifest["status"], "FAILED")
        self.assertIsNone(manifest.get("failure_step"), 
                         "failure_step should be None for pre-pipeline failures")
        
        # Verify no steps completed
        completed_steps = [name for name, data in manifest.get("steps", {}).items() 
                          if data.get("status") == "COMPLETED"]
    ...
    def test_failure_step_injected_during_step_execution(self):
        ...
        try:
            ...
            with patch("harness.asr.run_asr"), \
                 patch.object(runner, "_export_partial_bundle"), \
                 patch("harness.meeting_pack.build_meeting_pack"):
            ...
            self.assertEqual(manifest["status"], "FAILED")
            self.assertEqual(manifest["failure_step"], "asr", 
