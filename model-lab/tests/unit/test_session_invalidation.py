...
def test_invalidation_when_ingest_config_changes(tmp_path: Path, test_wav_path: Path) -> None:
    ...
    r1 = SessionRunner(
        test_wav_path,
        out_dir,
        preprocessing=IngestConfig(normalize=False),
        resume=True,
        steps=["ingest"],
    )
    m1 = r1.run()
    ...
    r2 = SessionRunner(
        test_wav_path,
        out_dir,
        preprocessing=IngestConfig(normalize=True),
        resume=True,
        config={"resume_from": run_dir},
        steps=["ingest"],
    )
    m2 = r2.run()
    ...
    assert res_cfg["normalize"] is True, (
        "Ingest should have re-run with new config (normalize=True)"
    )
