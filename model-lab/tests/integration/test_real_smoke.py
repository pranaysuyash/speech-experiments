...
def test_real_smoke_ingest_asr(tmp_path: Path, smoke_mp4_path: Path) -> None:
    ...
    assert ingest_step_2["started_at"] == t1_ingest_start, (
        "Ingest should have been skipped (same started_at)"
    )
    assert asr_step_2["started_at"] == t1_asr_start, (
        "ASR should have been skipped (same started_at)"
    )
