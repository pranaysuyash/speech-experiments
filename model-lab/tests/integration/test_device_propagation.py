...
class TestDevicePropagation:
    def test_device_preference_is_persisted_in_manifest(self, tmp_path, monkeypatch):
        ...
        data = {
            ...
            "config": json.dumps({"device_preference": ["mps", "cpu"], "some_misc_flag": True}),
        }
    ...
    def test_device_preference_reaches_asr_model_load(self, monkeypatch, tmp_path):
        ...
        load_model_mock = MagicMock(
            return_value={
                ...
                "capabilities": ["asr"],
                "asr": {
                    "transcribe": MagicMock(return_value={"segments": [], "info": MagicMock()})
                },
            }
        )
        ...
        monkeypatch.setattr(
            asr,
            "ingest_media",
            lambda i, o, c: {"processed_audio_path": str(i), "audio_content_hash": "abc"},
        )
        ...
        asr.run_asr(
            input_path=input_path, output_dir=output_dir, config=config, progress_callback=None
        )
