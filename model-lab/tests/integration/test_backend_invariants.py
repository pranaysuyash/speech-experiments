...
class TestBackendInvariants:
    ...
    def test_kill_run_idempotence(self, session_dir, mock_manifest):
        ...
        with (
            patch("server.services.runs_index.get_index") as mock_get_index,
    ...
    def test_retry_idempotence(self, monkeypatch, tmp_path):
        ...
        - Resets terminal status + error fields
        - Invalidates failed step + downstream artifacts
        - Preserves completed step records
        - Is idempotent (doesn't spawn multiple workers / re-mutate state)
        """
        ...
        manifest = {
            ...
            "steps": {
                ...
                "bundle": {
                    "status": "PENDING",
                    "artifacts": [{"path": "bundle/transcript.txt", "hash": "deadbeef"}],
                    "warnings": [],
                },
            },
        }
        ...
        assert m1["steps"]["ingest"]["artifacts"] == [
            {"path": ingest_artifact_rel, "hash": ingest_hash}
        ]
        ...
        # Idempotence: second retry does not spawn a new worker and preserves the manifest invariants.
        ...
        assert m2["steps"]["asr"]["status"] == "PENDING"
        assert m2["steps"]["asr"]["artifacts"] == []
    ...
    def test_asr_config_persistence(self, session_dir):
        ...
        user_config = {
            "asr": {"model_type": "faster_whisper", "device": "mps", "model_name": "default"}
        }

        # FIXED: Removed session_id arg
        runner = SessionRunner(
            input_path=input_path, output_dir=session_dir.parent, config=user_config
        )
    ...
    def test_failure_propagation(self, session_dir):
        ...
        runner = SessionRunner(input_path=input_path, output_dir=session_dir.parent)
        runner._init_dirs()
    ...
    def test_status_regression_prevention(self, session_dir):
        ...
        with patch("harness.session.ingest_media") as mock_ingest:
            mock_ingest.return_value = {
                "duration_s": 1.0,
                "processed_audio_path": str(input_path),
    ...
    def test_status_regression_prevention_strict(self, session_dir):
        ...
        assert (
            m_final["error"]["type"] == "UserCancelled"
        )  # Merge prevention (runner error didn't overwrite)
