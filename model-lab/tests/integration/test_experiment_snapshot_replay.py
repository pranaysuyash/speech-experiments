...
from server.api.candidates import CANDIDATES, Candidate
...
def test_experiment_snapshot_is_immutable_and_detects_tampering(mock_experiments_root):
    ...
    with mock.patch.dict(CANDIDATES, {GOLDEN_ID: MODIFIED_CANDIDATE}):
        ...
        assert data.get("provenance_status") == "VERIFIED", (
            f"Provenance status was: {data.get('provenance_status')}"
        )
    ...
    req_path.write_text(
        json.dumps(content)
    )  # Saving non-canonically might also trigger hash fail, which is good.
