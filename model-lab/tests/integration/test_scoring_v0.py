...
def test_scoring_with_transcript_txt(tmp_path):
    ...
    (session_dir / "bundle" / "action_items.csv").write_text(
...
def test_scoring_with_transcript_json(tmp_path):
    ...
    transcript_data = {
        "segments": [
            {
                "text": "This is a long enough transcript with substantial content. ",
                "start_s": 0.0,
                "end_s": 1.0,
            },
            {
                "text": "It has multiple segments that will be joined together. ",
                "start_s": 1.0,
                "end_s": 2.0,
            },
            {
                "text": "Each segment contains text to ensure we exceed the threshold. ",
                "start_s": 2.0,
                "end_s": 3.0,
            },
        ]
    }
    (session_dir / "bundle" / "transcript.json").write_text(
...
def test_api_enrichment(client, run_dir):
    ...
    (exp_dir / "experiment_request.json").write_text(
        json.dumps(
            {
                "schema_version": "2",
    ...
    (exp_dir / "experiment_state.json").write_text(
        json.dumps(
            {
                "schema_version": "1",
                "experiment_id": exp_id,
