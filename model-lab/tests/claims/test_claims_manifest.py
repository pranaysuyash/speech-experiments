...
class TestClaimsManifest:
    ...
    def test_claims_schema_valid(self, claims_path):
        ...
        for claim in data['claims']:
            ...
            for field in REQUIRED_CLAIM_FIELDS:
                assert field in claim, f"Missing required field '{field}' in claim: {claim.get('id', 'unknown')}"
            
            # Validate type
            assert claim['type'] in VALID_TYPES, f"Invalid type '{claim['type']}' in {claim['id']}"
            
            # Validate enforcement
            assert claim['enforcement'] in VALID_ENFORCEMENT, f"Invalid enforcement '{claim['enforcement']}' in {claim['id']}"
    ...
    def test_test_refs_follow_convention(self, claims_path):
        ...
        for claim in data['claims']:
            ...
            assert parts[1] == claim['task'], f"test_ref task mismatch: {parts[1]} != {claim['task']}"
