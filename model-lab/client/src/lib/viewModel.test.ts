...
describe('deriveRunDetailViewModel', () => {
    ...
    describe('failure reasons', () => {
        ...
        it('should add staleness as secondary reason when failed and stale', () => {
            ...
            expect(vm.secondaryReason).toBe('Worker stopped responding (No heartbeat)');
        });
    ...
    describe('pipeline steps', () => {
        ...
        it('should detect completed steps by artifacts when current_step is provided', () => {
            ...
            const run = createRun({ 
                ...
                current_step: 'diarization',
                steps: [
                    { name: 'ingest', status: 'COMPLETED', artifacts: [{ id: '1', filename: 'audio.wav', role: 'audio', produced_by: 'ingest', size_bytes: 1000, downloadable: true }] },
                    { name: 'asr', status: 'COMPLETED', artifacts: [{ id: '2', filename: 'transcript.txt', role: 'transcript', produced_by: 'asr', size_bytes: 500, downloadable: true }] }
                ]
            });
            const vm = deriveRunDetailViewModel(run);
            ...
            const run = createRun({
                ...
                steps: [
                    { 
                        ...
                        status: 'COMPLETED',
                        artifacts: [],
                        resolved_config: { model_id: 'whisper', source: 'openai', device: 'cuda', language: 'en' }
                    }
                ]
            });
            const vm = deriveRunDetailViewModel(run);
            ...
            const run = createRun({
                ...
                steps: [
                    { 
                        ...
                        status: 'COMPLETED',
                        artifacts: [],
                        resolved_config: { model_id: 'whisper', source: 'openai', device: 'mps', language: 'en' }
                    }
                ]
            });
            const vm = deriveRunDetailViewModel(run);
    ...
    describe('edge cases', () => {
        it('should handle empty steps_completed array', () => {
            ...
            expect(vm.pipelineSteps.every(s => s.status === 'PENDING' || s.status === 'UNKNOWN')).toBe(true);
        });
        ...
        it('should handle failure at last step', () => {
            const run = createRun({ 
                status: 'FAILED',
                failure_step: 'bundle',
                steps_completed: ['ingest', 'asr', 'diarization', 'alignment', 'chapters', 'summarize_by_speaker', 'action_items_assignee']
            });
            const vm = deriveRunDetailViewModel(run);
