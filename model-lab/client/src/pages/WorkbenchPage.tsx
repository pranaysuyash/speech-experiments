...
export default function WorkbenchPage() {
  ...
  const saveUserTemplate = async () => {
    ...
    try {
      ...
      showToast(
        'success',
        'Template Saved',
        `Saved "${newTemplateName.trim()}" with ${selectedSteps.length} steps`,
      );
    } catch (e) {
  ...
  const deleteUserTemplate = async (name: string) => {
    try {
      ...
      showToast('success', 'Template Deleted', `Deleted "${name}"`);
    } catch (e) {
    ...
    (async () => {
      try {
        ...
        if (cands.length > 0) {
          ...
          setCandidateIdB(
            cands.length > 1 ? cands[1].candidate_id : cands[0].candidate_id,
          );
        } else {
          setCandidateId('');
          setCandidateIdA('');
  ...
  const estimatedDuration = useMemo(() => {
    const stepsToUse =
      pipelineMode === 'custom' && resolvedSteps.length > 0
        ? resolvedSteps
        : selectedSteps;
  ...
  async function onStart() {
    ...
    try {
      const candsToRun =
        mode === 'single' ? [candidateId] : [candidateIdA, candidateIdB];

      // Runtime Invariants (Option A)
      if (mode === 'single' && (candsToRun.length !== 1 || !candsToRun[0])) {
        throw new Error(
          'Invariant Violation: Single mode requires exactly 1 selected candidate.',
        );
      }
      if (
        mode === 'compare' &&
        (candsToRun.length !== 2 || !candsToRun[0] || !candsToRun[1])
      ) {
        throw new Error(
          'Invariant Violation: Compare mode requires exactly 2 selected candidates.',
        );
      }
      ...
      const pipelineOptions =
        showAdvanced && pipelineMode !== 'preset'
          ? {
              steps: pipelineMode === 'custom' ? selectedSteps : undefined,
              preprocessing:
                selectedPreprocessing.length > 0
      ...
      if (mode === 'single') {
        ...
        if (result && result.run_id) {
          showToast('success', 'Run Started', `Processing ${file.name}`);
          navigate(`/runs/${result.run_id}`);
        } else {
          showToast(
            'success',
            'Experiment Created',
            'Redirecting to experiment details',
          );
        ...
        showToast(
          'success',
          'Comparison Started',
          `Running ${candsToRun.length} candidates`,
        );
        navigate(`/lab/experiments/${expId}`);
      }
    } catch (e: any) {
      ...
      const msg =
        data?.error_message || data?.detail || e?.message || 'Failed to start.';
      setError(msg);
      showToast('error', 'Failed to Start', msg);
      ...
      <div className='space-y-6 bg-white p-6 rounded-lg border shadow-sm'>
        ...
        <div>
          ...
          <DragDropFileUpload
            ...
            selectedFile={file}
            disabled={isSubmitting}
            maxSizeBytes={SIZE_WARNING_BYTES}
          />
        </div>
        ...
        <div className='border-t pt-4'>
          {mode === 'single' ? (
            ...
            <div className='grid grid-cols-2 gap-4'>
              <label className='block'>
                <span className='block font-semibold mb-1 text-blue-600'>
                  Candidate A
                </span>
                <select
                  value={candidateIdA}
              ...
              <label className='block'>
                <span className='block font-semibold mb-1 text-purple-600'>
                  Candidate B
                </span>
                <select
                  value={candidateIdB}
          ...
          {showAdvanced && (
            <div className='mt-4 space-y-4 p-4 bg-gray-50 rounded-lg'>
                  ...
                  <div className='space-y-3'>
                          ...
                          >
                            ...
                            <div className='flex flex-col'>
                              ...
                              <span className='text-gray-400 text-[10px]'>
                                {step.duration_estimate_s}s/min
                              </span>
                            </div>
                          </label>
                        ))}
                    ...
                    <div className='flex flex-wrap items-center gap-2 pt-2 border-t'>
                      ...
                      {userTemplates.map((t) => (
                        <div
                          key={t.name}
                          className='flex items-center gap-1 bg-purple-50 border border-purple-200 rounded px-2 py-1 text-xs'
                        >
                          <button
                  ...
                  <div className='space-y-3 border rounded p-3 bg-white'>
                    ...
                    {selectedSteps.includes('asr') && (
                      <div className='border-b pb-3'>
                        <span className='block font-medium text-xs text-gray-600 mb-2'>
                          ASR (Speech-to-Text)
                        </span>
                      ...
                      <div className='border-b pb-3'>
                        ...
                        <label className='block'>
                          ...
                          <select
                            ...
                            className='w-full border p-1.5 rounded text-xs'
                          >
                            <option value='heuristic_diarization'>
                              Heuristic (fast)
                            </option>
                            <option value='pyannote_diarization'>
                              Pyannote (accurate)
                            </option>
                          </select>
                        </label>
              ...
              <div className='text-xs text-gray-500 mt-2'>
                These settings override the candidate defaults. Model size
        ...
        <button
          ...
          disabled={
            !file ||
            isSubmitting ||
            (mode === 'single' && !candidateId) ||
            (mode === 'compare' && (!candidateIdA || !candidateIdB))
          }
