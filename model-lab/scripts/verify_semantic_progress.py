...
def test_semantic_progress():
    ...
    runner = SessionRunner(
        input_path=str(input_file), output_dir="runs/test_progress", steps=["mock_step"]
    )
    ...
    runner.steps["mock_step"] = StepDef(
        name="mock_step", deps=[], func=mock_asr_progress, artifact_paths=lambda x: []
    )
    ...
    # Run in thread
    t = threading.Thread(
        target=runner._execute_step, args=(runner._default_manifest(), runner.steps["mock_step"])
    )
    t.start()
