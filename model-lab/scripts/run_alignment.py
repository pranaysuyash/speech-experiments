...
from harness.alignment import align_artifacts, AlignedTranscript
from harness.nlp_schema import NLPArtifact, NLPRunContext, NLPInputs, NLPProvenance, compute_file_hash, validate_nlp_artifact
...
def run_dependency(script_name: str, input_path: Path, args_list: list = []) -> Path:
    ...
    cmd = [sys.executable, str(Path(__file__).parent / script_name), "--input", str(input_path)] + args_list
...
def run_alignment_pipeline(
    ...
    asr_artifact_path: Optional[Path] = None,
    diarization_artifact_path: Optional[Path] = None,
    ...
    if input_path:
        if not asr_artifact_path:
            ...
            asr_artifact_path = run_dependency("model_app.py", input_path, ["recommend", "--task", "asr", "--audio", str(input_path)] + (["--pre", pre] if pre else []))
    ...
    inputs = NLPInputs(
        ...
        asr_text_hash=compute_file_hash(diarization_artifact_path), # Hacking this field to store diarization hash?
...
def main():
    ...
    parser.add_argument("--diarization-artifact", type=Path, help="Pre-computed Diarization artifact")
