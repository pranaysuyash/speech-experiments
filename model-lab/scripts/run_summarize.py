...
from harness.chunking import Chunk, ChunkingPolicy, ChunkingResult, chunk_transcript
from harness.llm_provider import ErrorCode, LLMResult, get_llm_completion
...
def summarize_chunk(
    ...
    prompt = SUMMARY_PROMPT_TEMPLATE.format(
        max_sentences=max_sentences, transcript=chunk.text[:15000]
    )
    ...
    summary_output = SummaryOutput(
        ...
        compression_ratio=chunk.word_count / sum(len(s.split()) for s in sentences)
        if sentences
        else 0,
    )
...
def aggregate_summaries(
    ...
    if not all_sentences:
        return LLMResult(
            success=False, error_code=ErrorCode.INVALID_RESPONSE, error_message="No chunk summaries"
        ), []
    ...
    prompt = AGGREGATE_PROMPT_TEMPLATE.format(
        max_sentences=max_sentences, chunk_summaries=chunk_summaries
    )
...
def run_summarize(
    ...
    if not chunk_result.chunking_required:
        return _run_summarize_single(
            asr_artifact_path, asr_artifact, asr_artifact_hash, view, max_sentences, nlp_model
        )
...
def _run_summarize_single(
    ...
    prompt = SUMMARY_PROMPT_TEMPLATE.format(
        max_sentences=max_sentences, transcript=view.full_text[:15000]
    )
    ...
    summary_output = SummaryOutput(
        ...
        compression_ratio=view.word_count / sum(len(s.split()) for s in sentences)
        if sentences
        else 0,
    )
    ...
    artifact = NLPArtifact(
        ...
        errors=[f"{llm_result.error_code}: {llm_result.error_message}"]
...
def _run_summarize_chunked(
    ...
    if success_ratio < MIN_SUCCESS_RATIO:
        ...
        status = "failed"
        sentences: list[str] = []
        agg_result = LLMResult(
            success=False,
            error_code="CHUNK_FAILURE",
            error_message=f"Only {success_ratio:.0%} success",
        )
    elif not successful_chunks:
        status = "failed"
        sentences: list[str] = []
        agg_result = LLMResult(
            success=False, error_code="CHUNK_FAILURE", error_message="No successful chunks"
        )
    else:
    ...
    artifact = NLPArtifact(
        ...
        errors=[f"{agg_result.error_code}: {agg_result.error_message}"]
