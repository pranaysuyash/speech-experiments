...
from harness.model_card import ModelCard, EvidenceGrade
...
class ModelCoverage:
    ...
    use_case_eligibility: Dict[str, str] = field(default_factory=dict)  # use_case -> eligible/blocked/reason
...
def get_model_coverage(card: ModelCard, use_cases: List[Dict], decisions: Dict) -> ModelCoverage:
    ...
    for evidence in card.evidence:
        ...
        new_rank = grade_rank.get(evidence.evidence_grade.value if hasattr(evidence.evidence_grade, 'value') else str(evidence.evidence_grade), 0)
        if new_rank > current_rank:
            te.best_grade = evidence.evidence_grade.value if hasattr(evidence.evidence_grade, 'value') else str(evidence.evidence_grade)
    ...
    for uc in use_cases:
        ...
        if card.model_id in pipeline.values():
            ...
            if missing:
                ...
                coverage.use_case_eligibility[uc_id] = "⚠️ not selected (evidence insufficient or competitor better)"
...
def write_markdown_report(coverages: List[ModelCoverage], output_path: Path):
    ...
    for cov in coverages:
        ...
        best = "golden_batch" if "golden_batch" in grades else "smoke" if "smoke" in grades else "none"
        
        lines.append(f"| {cov.model_id} | {cov.model_type} | {declared} | {evidence_str} | {best} |")
...
def main():
    ...
    print(f"✅ Generated docs/COVERAGE.md")
    
    write_json_report(coverages, docs_dir / "coverage.json")
    print(f"✅ Generated docs/coverage.json")
