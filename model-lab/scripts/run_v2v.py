...
def run_v2v_adhoc(
    model_id: str, input_path: str, prompt: str = None, device: str = "cpu", pre: str = None
):
    """
    ...
    try:
        ...
        t0 = time.time()
        output = (
            v2v_fn(audio=audio_for_model, prompt=prompt)
            if prompt
            else v2v_fn(audio=audio_for_model)
        )
        ...
        eval_result = V2VMetrics.evaluate(
            response_audio=response_audio, sr=sr_out, latency_s=latency
        )
...
def save_run_artifact(
    ...
    artifact = {
        "meta": {"run_id": run_id, "timestamp": timestamp, "model_id": model_id, "task": "v2v"},
...
def main():
    ...
    if args.audio:
        try:
            result, artifact_path = run_v2v_adhoc(
                args.model, str(args.audio), prompt=args.prompt, device=args.device, pre=args.pre
            )
    ...
    for case in cases:
        ...
        try:
            ...
            rtf_like = (
                (latency * 1000) / (input_duration_s * 1000) if input_duration_s > 0 else None
            )
            ...
            results.append(
                {
                    ...
                    "metrics": {
                        ...
                        "rtf": latency / eval_result.response_duration_s
                        if eval_result.response_duration_s > 0
                        else 0,
                    },
                }
            )
    ...
    if valid_results:
        ...
        avg_rtf_like = np.mean(
            [
                r["metrics"]["rtf_like"]
                for r in valid_results
                if r["metrics"].get("rtf_like") is not None
            ]
        )
        avg_metrics = {
            ...
            "response_duration_s": float(
                np.mean([r["metrics"]["response_duration_s"] for r in valid_results])
            ),
    ...
    summary = {"device": args.device, "metrics": avg_metrics, "gates": {"has_failure": has_failure}}

    # Get dataset path for provenance
    dataset_path = Path(f"data/golden/{args.dataset}.yaml")

    save_run_artifact(
        args.model, args.dataset, dataset_path, results, summary, Path(f"runs/{args.model}/v2v")
    )
