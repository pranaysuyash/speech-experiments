...
def run_tts_test(model_id: str, dataset: str, device: str = None):
    ...
    for prompt in prompts:
        ...
        if audio_health['has_failure']:
            all_healthy = False
            print(f"  ⚠️ Audio health failed: {[k for k, v in audio_health.items() if k.startswith('is_') or k.startswith('has_') and v]}")
        else:
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='tts_smoke_v1',
                       help='Dataset ID (default: tts_smoke_v1)')
