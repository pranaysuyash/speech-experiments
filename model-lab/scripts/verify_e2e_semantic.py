...
def setup_run():
    ...
    runner = SessionRunner(
        input_path=str(input_file), output_dir=f"runs/{run_id}", steps=["mock_long"]
    )

    # Inject mock step
    runner.steps["mock_long"] = StepDef(
        name="mock_long", deps=[], func=mock_step_long, artifact_paths=lambda x: []
    )
    ...
    # Run in thread
    t = threading.Thread(
        target=runner._execute_step, args=(runner._default_manifest(), runner.steps["mock_long"])
    )
    t.start()
