...
def validate_promotion(
    run: dict, target_grade: str, gt_path: Path | None = None
) -> tuple[bool, str]:
    """
...
def promote_run(run_path: Path, target_grade: str, gt_path: Path | None = None) -> bool:
    """
...
def main():
    ...
    parser.add_argument(
        "--to-grade", required=True, choices=["smoke", "golden_batch"], help="Target grade"
    )
    parser.add_argument("--with-gt", type=Path, help="Ground truth path (for golden)")
    parser.add_argument(
        "--force", action="store_true", help="Force promotion without validation (use with caution)"
    )
