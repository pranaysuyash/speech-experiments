...
def setup_test_run():
    ...
    manifest = {
      "run_id": RUN_ID,
      "status": "RUNNING",
      "started_at": time.strftime("%Y-%m-%dT%H:%M:%S+00:00", time.gmtime(time.time() - 120)), # 2 mins ago
      "updated_at": time.strftime("%Y-%m-%dT%H:%M:%S+00:00", time.gmtime(time.time() - 40)), # 40s ago (Should trigger "Process may still be alive")
