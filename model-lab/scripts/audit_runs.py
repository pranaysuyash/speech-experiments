...
def audit_run_manifest(manifest_path):
    ...
    try:
        ...
        if status == "RUNNING":
            if not updated_at:
                issues.append(f"RUNNING but no updated_at")
            else:
                try:
                    ...
                    if elapsed > 300: 
                        issues.append(f"RUNNING but no heartbeat for {int(elapsed)}s (Stalled candidate)")
        ...
        if status == "STALLED":
             issues.append(f"Manifest has explicit STALLED status (should be derived from RUNNING)")
        ...
        if status == "FAILED" and not failure_step:
            ...
            # Actually user asked: "Capture failure_step deterministically"
             issues.append(f"FAILED but failure_step is missing or null")
