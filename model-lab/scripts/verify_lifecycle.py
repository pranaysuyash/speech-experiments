...
def main():
    ...
    r = requests.post(
        f"{BASE_URL}/api/workbench/runs",
        files=files,
        data={"use_case_id": "default", "steps_preset": "full"},
    )
    ...
    check(
        status is not None and status["status"] == "RUNNING",
        f"Run status is RUNNING (got {status['status']})",
    )
