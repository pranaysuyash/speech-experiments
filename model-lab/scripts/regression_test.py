...
class RegressionTester:
    ...
    def __init__(
        self, results_dir: str = "runs", baseline_file: str = "runs/regression_baseline.json"
    ):
    ...
    def run_golden_tests(self, model_type: str, test_data_path: str) -> dict[str, Any]:
        ...
        results = {"model_type": model_type, "timestamp": datetime.now().isoformat(), "metrics": {}}

        # Test ASR if applicable
        if (
            "asr" in model_type.lower()
            or "whisper" in model_type.lower()
            or "lfm" in model_type.lower()
        ):
    ...
    def _test_tts(self, model: Any, test_data_path: str) -> dict[str, float]:
        ...
        for case in test_cases[:5]:  # Test subset for speed
            ...
            try:
                ...
                mos = self._calculate_mos(
                    generated_audio, reference_audio if reference_audio else None
                )
        ...
        logger.info(
            f"TTS Results: MOS={avg_mos:.2f}, Similarity={avg_similarity:.2f} ({len(mos_scores)} samples)"
        )
    ...
    def compare_to_baseline(
        ...
        comparison = {"passed": True, "failures": [], "improvements": [], "metric_deltas": {}}
                ...
                if metric in ["wer", "cer"]:  # Higher is worse
                    if delta > threshold:
                        ...
                        comparison["improvements"].append(
                            f"{metric.upper()} improved by {-delta:.3f}"
                        )
    ...
    def run_regression_test(
        self, model_type: str, test_data_path: str = "data", update_baseline: bool = False
    ) -> int:
        ...
        if baseline:
            ...
            result_file = (
                self.results_dir
                / f"regression_{model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
...
def main():
    ...
    exit_code = tester.run_regression_test(
        args.model_type, test_data_path=args.test_data, update_baseline=args.update_baseline
    )
