...
from harness.chapters import SemanticSegmenter, Chapter
from harness.alignment import AlignedTranscript, AlignedSegment, AlignmentMetrics, load_alignment
from harness.llm_provider import get_llm_completion
from harness.nlp_schema import NLPArtifact, compute_file_hash
...
def enrich_chapter(chapter: Chapter, context: str):
    ...
    cache_dir = Path("runs/cache/embeddings") if not os.environ.get("EMBEDDING_CACHE_DIR") else Path(os.environ.get("EMBEDDING_CACHE_DIR"))
...
def run_chapters(alignment_path: Path, cache_dir: Optional[Path] = None):
...
def main():
    ...
    parser.add_argument("--embedding-cache-dir", type=Path, help="Cache directory for embedding models")
