...
from harness.model_card import ModelCard, TaskType, EvidenceGrade
...
def evaluate_gate(gate_name: str, evidence: Any, use_case: Dict[str, Any]) -> GateEvaluation:
    ...
    if gate_name == "wer_valid":
        has_invalid_wer = any(
            "❌" in str(ev.gates.get("wer_valid", ""))
            for ev in evidence
        )
        if has_invalid_wer:
            if gate_name in fatal_gates:
                return GateEvaluation(gate_name, Severity.FAIL_FATAL, "WER outside acceptable bounds")
            elif gate_name in warning_gates:
                return GateEvaluation(gate_name, Severity.WARN, "WER degraded (conversational tolerance)")
            else:
    ...
    if gate_name == "is_truncated":
        has_truncation = any(
            ev.gates.get("is_truncated") == "❌ Truncated"
            for ev in evidence
        )
        if has_truncation:
            if gate_name in fatal_gates:
                ...
                return GateEvaluation(gate_name, Severity.WARN, "Output truncated (acceptable for conversational)")
        return GateEvaluation(gate_name, Severity.PASS, "Output complete")
    
    # Check coverage
    if gate_name == "coverage_low":
        has_low_coverage = any(
            ev.metrics.get("coverage_ratio", 1.0) < 0.6
            for ev in evidence
        )
        if has_low_coverage:
    ...
    if gate_name == "latency_high":
        ...
        if max_latency:
            has_high_latency = any(
                ev.metrics.get("latency_ms_p50", 0) > max_latency
                for ev in evidence
            )
            if has_high_latency:
                if gate_name in fatal_gates:
                    return GateEvaluation(gate_name, Severity.FAIL_FATAL, f"Latency > {max_latency}ms")
                else:
    ...
    if gate_name == "audio_empty":
        has_empty_audio = any(
            ev.metrics.get("audio_duration_s", 1.0) == 0
            for ev in evidence
        )
        if has_empty_audio:
    ...
    if gate_name == "speaker_error_high":
        has_speaker_error = any(
            abs(ev.metrics.get("speaker_count_error", 0)) > 1
            for ev in evidence
        )
    ...
    for req in primary_reqs:
        ...
        try:
            ...
            if grade_rank.get(grade.value, 0) < grade_rank.get(min_grade.value, 0):
                ...
                fatal_gates.append(f"{task}: evidence grade {grade.value} < required {min_grade.value}")
                continue
...
def _count_valid_runs_for_model(card: ModelCard, task: str, min_grade: str) -> int:
    ...
    for evidence in card.evidence:
        ...
        ev_grade = evidence.evidence_grade.value if hasattr(evidence.evidence_grade, 'value') else str(evidence.evidence_grade)
...
def _generate_explanation(outcome: Outcome, pass_gates: List[str], warn_gates: List[str], fatal_gates: List[str]) -> str:
...
def generate_decisions_doc(results: Dict[str, List[DecisionResult]]):
    ...
    for use_case_name, decisions in results.items():
        ...
        for d in sorted_decs:
            icon = "✅" if d.outcome == Outcome.RECOMMENDED else "⚠️" if d.outcome == Outcome.ACCEPTABLE else "❌"
...
def select_best_model_for_task(
    ...
    for card in cards:
        ...
        if task == "asr":
            ...
            spk_errs = [abs(e.metrics.get("speaker_count_error", 0)) for e in evidence 
            ...
            speech_ratios = [e.metrics.get("speech_ratio") for e in evidence 
            ...
            if rtf_likes:
                ...
                latencies = [e.metrics.get("latency_ms") for e in evidence 
...
def evaluate_pipeline_for_use_case(cards: List[ModelCard], use_case: Dict[str, Any]) -> PipelineResult:
    """
    ...
    for req in primary_reqs:
        ...
        if model_id is None:
            if op_rejections:
                ...
                fatal_reasons.append(f"Primary task {task}: operability failed - {'; '.join(op_rejections[:2])}")
            else:
                fatal_reasons.append(f"No viable model for primary task: {task} (min_grade: {min_grade})")
        else:
...
def _count_valid_runs_for_task(cards: List[ModelCard], model_id: str, task: str, min_grade: str) -> int:
...
def _count_distinct_datasets_for_task(cards: List[ModelCard], model_id: str, task: str, min_grade: str) -> int:
...
def _generate_pipeline_explanation(outcome: Outcome, components: List[PipelineComponent], 
                                   fatal_reasons: List[str], warn_reasons: List[str]) -> str:
    ...
    if components:
        ...
        for c in components:
            ...
            lines.append(f"  • {c.task}: {c.model_id} ({c.evidence_grade}, {c.metric_name}={metric_str})")
...
def generate_pipeline_decisions_doc(results: Dict[str, PipelineResult], single_model_results: Dict[str, List[DecisionResult]]):
    ...
    for use_case_name, decisions in single_model_results.items():
        ...
        sorted_decs = sorted(decisions, key=lambda x: (x.outcome == Outcome.RECOMMENDED, x.outcome == Outcome.ACCEPTABLE, x.score), reverse=True)
        ...
        for d in sorted_decs:
            ...
            if d.outcome == Outcome.REJECTED:
                ...
                detail = f"{len(d.pass_gates)} strengths, {len(d.warn_gates)} warnings"
            lines.append(f"| **{d.model_id}** | {icon} {d.outcome.name} | {d.score:.1f} | {detail} |")
    ...
    for use_case_name, result in results.items():
        ...
        icon = "✅" if result.outcome == Outcome.RECOMMENDED else "⚠️" if result.outcome == Outcome.ACCEPTABLE else "❌"
...
def generate_decisions_json(pipeline_results: Dict[str, PipelineResult], 
    ...
    for uc_name, results in single_model_results.items():
        ...
        decisions["use_cases"][uc_name] = {
            ...
            "acceptable": acceptable,
            "rejected": rejected,
            "best_model": recommended[0] if recommended else (acceptable[0] if acceptable else None),
            "models": [
                {
                    "model_id": d.model_id,
    ...
    for card in cards:
        for ev in card.evidence:
            ...
            grade = ev.evidence_grade if isinstance(ev.evidence_grade, str) else ev.evidence_grade.value
    ...
    for task, models in task_models.items():
        ...
        by_grade = sorted(unique_models, key=lambda x: grade_rank.get(x["evidence_grade"], 0), reverse=True)
