...
from harness.model_card import (
    ModelCard,
)
from harness.registry import ModelRegistry
from harness.taxonomy import EvidenceGrade
...
def load_all_task_evidence(model_id: str) -> dict[str, dict[str, Any] | None]:
    """
...
def load_best_run_for_task(model_id: str, task: str) -> dict[str, Any] | None:
    """
    ...
    runs_data.sort(key=lambda r: (is_valid(r), grade_score(r), get_ts(r)), reverse=True)
    ...
    if "golden" in grade_str:
        grade_enum = EvidenceGrade.GOLDEN_BATCH
    elif "smoke" in grade_str:
        grade_enum = EvidenceGrade.SMOKE
    else:
        grade_enum = EvidenceGrade.ADHOC

    result = {
        "task": task,
        "dataset_id": evidence.get("dataset_id")
        or run_data.get("input", {}).get("dataset_id", "unknown"),
    ...
    if task == "asr":
        ...
        result["metrics"] = {"wer": wer, "rtf": rtf, "latency_ms": latency}
        result["gates"] = {
...
def load_runs_summary(model_id: str) -> dict[str, Any] | None:
...
def generate_markdown(cards: list[ModelCard], output_path: Path):
    ...
    lines.append("")
    lines.append("> Auto-generated from registry + configs + runs. Do not edit manually.")
    lines.append(f"> Generated from commit: {git_info['commit']} (tree: {git_info['tree']})")
    if git_info["dirty"]:
        lines.append("> ⚠️ Working directory has uncommitted changes")
    lines.append("")
    ...
    for card in sorted(cards, key=lambda c: (c.status != "production", c.model_id)):
        ...
        lines.append(
            f"| **{card.model_id}** | {status_emoji} {card.status} | {caps} | {runtimes} | {targets} | {wer} | {rtf} |"
        )
            ...
            for ev in sorted(card.evidence, key=lambda e: e.task):
                ...
                for k, v in ev.metrics.items():
                    ...
                    if isinstance(v, float):
                        if "rtf" in k:
                            val = f"{v:.2f}x"
                        elif "time" in k or "latency" in k:
                            val = f"{v:.0f}ms"
                        elif "wer" in k or "cer" in k:
                            val = f"{v:.1%}"
                        else:
                            val = f"{v:.2f}"
                    else:
                ...
                failures = [
                    ...
                    if (k == "has_failure" and v) or (k != "has_failure" and v is False)
                ]
                if failures:
                ...
                lines.append(
                    f"| {ev.task} | {ev.evidence_grade} | {ev.device or '-'} | {metrics_str} | {gates_str} | {ev.verified_at or '-'} |"
                )
            lines.append("")
    ...
    for use_case in use_cases:
        ...
        for card in cards:
            ...
            if card.observed.rtf_median:
                score += (
                    (1 - min(card.observed.rtf_median, 1)) * prefers.get("rtf_weight", 0.3) * 100
                )
