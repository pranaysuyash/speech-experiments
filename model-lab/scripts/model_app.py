...
def cmd_recommend(args):
    ...
    if getattr(args, "task", None):
        ...
        if not task_data:
            if args.json:
                print(
                    json.dumps(
                        {
                            "error": f"Unknown task: {args.task}",
                            "available": list(tasks.keys()) + list(nlp_tasks),
                        }
                    )
                )
            else:
        ...
        if args.json:
            ...
            icon = (
                "✅"
                if outcome.upper() == "RECOMMENDED"
                else "⚠️"
                if outcome.upper() == "ACCEPTABLE"
                else "❌"
            )
            print(f"\nTask: {args.task}")
            ...
            if models:
                ...
                for i, m in enumerate(models[:3], 1):
                    ...
                    print(
                        f"  {i}. {m.get('model_id')} (grade={m.get('evidence_grade')}, device={device})"
                    )
        ...
        if getattr(args, "audio", None) and best:
            ...
            if args.task == "summarize_from_asr":
                ...
                print("  Step 1: Running ASR...")
                asr_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_asr.py"),
                    "--model",
                    best,
                    "--input",
                ...
                print("  Step 2: Running summarize...")
                sum_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_summarize.py"),
                    "--from-artifact",
                    asr_artifact_path,
                ]
                ...
                if sum_result.returncode == 0:
                    ...
                    print("  ⚠️ Summarize failed (artifact may still exist)")
            ...
            if args.task == "action_items_from_asr":
                ...
                ai_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_action_items.py"),
                    "--from-artifact",
                    asr_artifact_path,
                ]
                ...
                if ai_result.returncode == 0:
                    ...
                    print("  ⚠️ Action items extraction failed")
                    if ai_result.stderr:
            ...
            if args.task == "ner_from_asr":
                ...
                ner_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_ner.py"),
                    "--from-artifact",
                    asr_artifact_path,
                ]
            ...
            if args.task == "alignment_from_asr":
                ...
                align_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_alignment.py"),
                    "--input",
                    str(audio_path.resolve()),
                ]
                if getattr(args, "pre", None):
            ...
            if args.task == "summarize_by_speaker_from_asr":
                ...
                sum_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_summarize_by_speaker.py"),
                    "--input",
                    str(audio_path.resolve()),
                ]
                if getattr(args, "pre", None):
            ...
            if args.task == "chapters_from_asr":
                ...
                chapters_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_chapters.py"),
                    "--input",
                    str(audio_path.resolve()),
                ]
                if getattr(args, "pre", None):
            ...
            if args.task == "action_items_with_assignee_from_asr":
                ...
                ai_cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / "scripts/run_action_items_with_assignee.py"),
                    "--input",
                    str(audio_path.resolve()),
                ]
                if getattr(args, "pre", None):
                ...
                if ai_result.returncode == 0 and ai_artifact_path:
                    ...
                    print("  ❌ Action Extraction failed")
                    if ai_result.stderr:
            ...
            if script:
                ...
                cmd = [
                    sys.executable,
                    str(Path(__file__).parent.parent / script),
                    "--model",
                    best,
                    "--audio",
                    str(audio_path.resolve()),
...
def cmd_status(args):
    ...
    # Collect status data
    status_data = {"models": {}, "evidence": {}, "decisions": {}}
    ...
    status_data["evidence"]["valid_runs"] = (
        len(list(runs_dir.glob("**/*.json"))) if runs_dir.exists() else 0
    )
    status_data["evidence"]["quarantined"] = (
        len(list(quarantine_dir.glob("**/*.json"))) if quarantine_dir.exists() else 0
    )
...
def cmd_claims(args):
    ...
    if args.model:
        ...
        if args.json:
            ...
            print("\nSummary:")
            print(
                f"  Enforced: {s['enforced_pass']} pass, {s['enforced_fail']} fail, {s['enforced_skip']} skip"
            )
            print(
                f"  Optional: {s['optional_pass']} pass, {s['optional_fail']} fail, {s['optional_skip']} skip"
            )
...
def cmd_sweep(args):
    ...
    if not coverage_path.exists():
        ...
        subprocess.run(
            [sys.executable, str(Path(__file__).parent / "coverage_report.py")], capture_output=True
        )

    with open(coverage_path) as f:
        coverage = json.load(f)

    results = {"success": [], "failed": [], "blocked": [], "skipped": []}
    ...
    for model_id in models:
        ...
        if missing_env:
            results["blocked"].append(
                {"model": model_id, "reason": f"Missing env: {', '.join(missing_env)}"}
            )
        ...
        for task in capabilities:
            ...
            if current_rank >= target_rank:
                results["skipped"].append(
                    {"model": model_id, "task": task, "reason": f"already has {best_grade}"}
                )
                continue
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    results["failed"].append(
                        {"model": model_id, "task": task, "error": result.stderr[-200:]}
                    )
    ...
    subprocess.run(
        [sys.executable, str(Path(__file__).parent / "generate_decisions.py")], capture_output=True
    )
...
""",
    ...
    p_recommend.add_argument(
        "--task", "-t", help="Task: asr, vad, diarization, v2v, chapters_from_asr, etc."
    )
    p_recommend.add_argument("--audio", "-a", type=Path, help="Audio file to run on")
    p_recommend.add_argument(
        "--prompt", "-p", help="Text prompt for V2V (only valid with --task v2v)"
    )
    ...
    p_run.add_argument(
        "--task", "-t", required=True, help="Task: asr, vad, diarization, v2v, alignment_from_asr"
    )
    ...
    p_sweep.add_argument(
        "--max-models", type=int, default=100, help="Max models to process (default: 100)"
    )
    p_sweep.add_argument(
        "--max-runs", type=int, default=50, help="Max runs to create (default: 50)"
    )
