...
def validate_model_config(model_id: str) -> tuple[bool, list[str], dict[str, Any] | None]:
...
def run_smoke_test(
    model_id: str, task: str, dataset: str, dry_run: bool = False
) -> tuple[bool, str]:
...
def main():
    ...
    if not env_ok:
        print("❌ Blocked by missing environment:")
        for e in env_errors:
            print(f"   - {e}")
    ...
    if not args.no_claims and not args.dry_run:
        ...
        if not claims_path.exists():
            print("❌ No claims.yaml found. Use --init-claims or --no-claims")
            sys.exit(EXIT_CONFIG_INVALID)
