...
class TestBackendFixes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from harness.session import SessionRunner, SessionContext
        self.runner = SessionRunner(
            input_path=self.dummy_input,
            output_dir=self.runs_root,
            force=True
        )
    ...
    def test_alignment_func_contract(self):
        ...
        with patch('harness.alignment.run_alignment', return_value=mock_path) as mock_run:
            ...
            self.assertIsInstance(result, dict, 
                                  f"alignment_func should return a dict, got {type(result)}")
    ...
    def test_stale_run_snapshot_consistency(self):
        ...
        old_time = (datetime.now(timezone.utc) - timedelta(seconds=200)).isoformat().replace("+00:00", "Z")
    ...
    def test_artifact_registry_and_lookup(self):
        ...
        self.assertEqual(min([x["path"] for x in m["artifacts_by_type"]["result"]]), "out.json") # Use min/list sort safety
    ...
    def test_stale_run_robustness(self):
        ...
        with patch("server.services.runs_index.RunsIndex.get_run") as mock_get:
            mock_get.return_value = {"run_id": run_id, "status": "RUNNING", "manifest_path": str(run_dir / "manifest.json"), "steps_completed": []}
