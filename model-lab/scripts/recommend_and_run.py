...
def get_best_model_for_task(task: str, policy: str = "strict") -> tuple:
    ...
    if recommended:
        ...
        return best["model_id"], "RECOMMENDED", f"Decision engine: RECOMMENDED ({best['evidence_grade']})", False
    
    # Find ACCEPTABLE models
    acceptable = [m for m in models if m["outcome"] == "acceptable"]
    if acceptable:
        best = acceptable[0]
        return best["model_id"], "ACCEPTABLE", f"Decision engine: ACCEPTABLE ({best['evidence_grade']}) - with trade-offs", False
    ...
    if best_by_grade:
        ...
        return best_by_grade, "BEST_EFFORT", f"⚠️ BEST_EFFORT: No models meet criteria. Using {best_by_grade} ({grade})", True
    ...
    if uc.get("evaluation_mode") == "pipeline":
        ...
        if outcome == "recommended":
            ...
            if policy == "best_effort":
                ...
                return pipeline, "BEST_EFFORT", f"⚠️ Pipeline incomplete: {'; '.join(uc.get('fatal_reasons', []))}", True
            return None, "REJECTED", f"Pipeline not viable: {'; '.join(uc.get('fatal_reasons', []))}", False
    else:
...
def run_task(task: str, model_id: str, audio_path: Path, **kwargs):
    ...
    if task == "asr":
        ...
        for turn in turns[:10]:
            print(f"  [{turn.get('start', 0):.1f}s - {turn.get('end', 0):.1f}s] {turn.get('speaker', '?')}")
        if len(turns) > 10:
        ...
        duration = result.get("duration_s", 0) or (len(output_audio) / output_sr if output_audio is not None else 0)
...
Examples:
  ...
  python scripts/recommend_and_run.py --use-case meeting_analysis --audio meeting.wav
    ...
    parser.add_argument("--use-case", type=str, dest="use_case",
                       help="Use case (runs pipeline)")
    parser.add_argument("--model", type=str, default=None,
                       help="Override model selection")
    parser.add_argument("--policy", type=str, default="strict",
                       choices=["strict", "best_effort"],
                       help="Selection policy (default: strict)")
    parser.add_argument("--include-in-arsenal", action="store_true",
                       help="Include run in arsenal (default: excluded)")
    parser.add_argument("--prompt", type=str, default=None,
                       help="Text prompt for TTS/V2V")
    ...
    if args.use_case:
        pipeline, outcome, reason, is_best_effort = get_pipeline_for_use_case(args.use_case, args.policy)
        ...
        if is_best_effort:
            print(f"   ⚠️  BEST_EFFORT MODE - Results not evidence-quality")
        print()
