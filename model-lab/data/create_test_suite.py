...
def generate_white_noise(duration: float, sample_rate: int = 16000, 
                          amplitude: float = 0.1) -> np.ndarray:
    """Generate white noise."""
    return amplitude * np.random.randn(int(sample_rate * duration))


def generate_pink_noise(duration: float, sample_rate: int = 16000, 
                       amplitude: float = 0.1) -> np.ndarray:
...
def generate_sine_sweep(start_freq: float, end_freq: float, duration: float, 
...
def generate_multitone(duration: float, sample_rate: int = 16000,
...
def generate_click_train(duration: float, sample_rate: int = 16000, 
                        click_rate: float = 10.0) -> np.ndarray:
...
def main():
    ...
    parser.add_argument('--sample-rate', type=int, default=16000,
                       help='Sample rate for generated files')
    parser.add_argument('--list-only', action='store_true',
                       help='List what would be generated')
